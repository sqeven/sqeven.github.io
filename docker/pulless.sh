...
if [ "$1" == search ];then
    ...
    curl -sX GET https://api.github.com/repos/icarephone/${repository}/contents/$img_name?ref=develop |
    ...
    if [[ "$img" =~ ^gcr.io|^quay.io ]];then
        repo=registry.cn-shenzhen.aliyuncs.com/sqeven/${name//\//.}
        ...
        repo=registry.cn-shenzhen.aliyuncs.com/sqeven/${name/k8s.gcr.io\//gcr.io.google_containers.}
        docker pull $repo:$tag
        docker tag $repo:$tag $img
        docker rmi $repo:$tag
    else
